...
CREATE TABLE cycles (
    ...
    INDEX idx_cycles_user_start_date (user_id, last_period_start_date)
);
...
CREATE TABLE symptoms (
    ...
    INDEX idx_symptoms_user_entry_date (user_id, entry_date),
...
CREATE TABLE moods (
    ...
    INDEX idx_moods_user_entry_date (user_id, entry_date)
);
...
CREATE TABLE flow_entries (
    ...
    user_id BIGINT NOT NULL,
    entry_date DATE NOT NULL,
    flow_level VARCHAR(30) NOT NULL,
    flow_color VARCHAR(30) NOT NULL,
    clot_size VARCHAR(20) NOT NULL,
    notes VARCHAR(500) NULL,
    created_at DATETIME NOT NULL,
    CONSTRAINT fk_flow_entries_user FOREIGN KEY (user_id) REFERENCES app_users(id) ON DELETE CASCADE,
    CONSTRAINT uk_flow_entries_user_date UNIQUE (user_id, entry_date),
    INDEX idx_flow_entries_user_entry_date (user_id, entry_date)
);
...
CREATE TABLE water_logs (
    ...
    INDEX idx_water_logs_user_entry_date (user_id, entry_date)
);
...
CREATE TABLE sleep_logs (
    ...
    INDEX idx_sleep_logs_user_entry_date (user_id, entry_date)
);
...
CREATE TABLE journal_entries (
    ...
    nutrition_log VARCHAR(3000) NULL,
    created_at DATETIME NOT NULL,
    CONSTRAINT fk_journal_entries_user FOREIGN KEY (user_id) REFERENCES app_users(id) ON DELETE CASCADE,
    INDEX idx_journal_entries_user_entry_date (user_id, entry_date)
);
